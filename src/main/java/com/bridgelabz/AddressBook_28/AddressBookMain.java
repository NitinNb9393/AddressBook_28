...
public class AddressBookMain {
	...
	Hashtable<String, ArrayList<contactInfo>> personInfoDict = new Hashtable<String, ArrayList<contactInfo>>();
	contactInfo p = null;
	ArrayList<contactInfo> pList = new ArrayList<contactInfo>();
	...
	/*
	 * Purpose : Using Java Streams to search for Person in a City or State across
	 * the multiple AddressBook. Maintain Dictionary of City and Person as well as
	 * State and Person Finally get the count of Persons by City or State using java
	 * streams
	 */
	...
	public void sortPerson() {
		...
		case 1:
			...
			personInfoDict.keySet().forEach(entry -> {
				List<contactInfo> data = personInfoDict.get(entry).stream()
						.sorted(Comparator.comparing(contactInfo::getFirst_name)).collect(Collectors.toList());
		...
		case 2:
			...
			personInfoDict.keySet().forEach(entry -> {
				List<contactInfo> data = personInfoDict.get(entry).stream()
						.sorted(Comparator.comparing(contactInfo::getCity)).collect(Collectors.toList());
				System.out.println(data);
			});
			break;
		case 3:
			...
			personInfoDict.keySet().forEach(entry -> {
				List<contactInfo> data = personInfoDict.get(entry).stream()
						.sorted(Comparator.comparing(contactInfo::getState)).collect(Collectors.toList());
				System.out.println(data);
			});
			break;
		case 4:
			...
			personInfoDict.keySet().forEach(entry -> {
				List<contactInfo> data = personInfoDict.get(entry).stream()
						.sorted(Comparator.comparing(contactInfo::getZip)).collect(Collectors.toList());
				System.out.println(data);
			});
			break;
		}
